...
---
    ...
    1. Internacionalització
        a) Introducció
        b) Biblioteca GNU de internacionalització: Gettext
        c) POEdit. Arxiu PO
        d) Detecció automàtica de l'idioma del navegador.
        e) Formulari de selecció d'idioma.
...
Aquí parlem de claus de traducció, hi ha dues escoles principals:
 ...
 1. msgstr "com a frase real. Els avantatges principals són:
    *    si hi ha peces del programari sense traduir en un llenguatge determinat, la tecla mostrada encara mantindrà algun significat. Exemple: si us convé traduir per cor de l’anglès al castellà però necessiteu ajuda per traduir al francès, podeu publicar la pàgina nova amb frases en francès que falten i, en canvi, algunes parts del lloc web es mostrarien en anglès;
     *   és molt més fàcil per al traductor entendre el que passa i fer una traducció adequada basant-se en la característica de configuració;
     *   et dóna l10n de franc per a un idioma: el de procedència;
     *   L’únic desavantatge: si cal canviar el text real, haureu de substituir el mateix msgid per diversos fitxers d’idiomes.
 2. msgstr "com a clau única i estructurada. Descriuria el paper de l’oració a l’aplicació de manera estructurada, inclosa la plantilla o la part on es troba la cadena en lloc del contingut.
    *    és una manera fantàstica d’organitzar el codi, separant el contingut del text de la lògica de la plantilla.
    *    tanmateix, això podria provocar problemes al traductor que faria faltar el context. Caldria un fitxer d’idioma font com a base d’altres traduccions. Exemple: el desenvolupador idealment tindria un fitxer en.po, que els traductors llegirien per entendre què escriure en fr.po per exemple.
     *   les traduccions que falten mostraran tecles sense sentit a la pantalla (top_menu.welcome en lloc de Hello there, User! a la dita pàgina en francès sense traduir). És bo, ja que obligaria a que la traducció estigui completa abans de publicar-la, però els problemes de traducció són notablement horribles a la interfície. Algunes biblioteques, però, inclouen una opció per especificar un idioma determinat com a "fallback", tenint un comportament similar a l'altre plantejament.

El manual de Gettext afavoreix el primer plantejament, ja que, en general, és més fàcil per a traductors i usuaris en cas de problemes. Així estarem treballant també aquí. Tanmateix, la documentació de Symfony afavoreix la traducció basada en paraules clau, per permetre canvis independents de totes les traduccions sense afectar també les plantilles.

# Poedit #

## Ús diari ##
En una aplicació típica, faríeu servir algunes funcions de Gettext mentre escriviu text estàtic a les vostres pàgines. Aquestes frases apareixerien a fitxers .po, es traduirien, es recopilen en fitxers .mo i, després, les utilitzava Gettext per a interpretar la interfície real. Tenint en compte això, anem a unir el que hem comentat fins ara en un exemple pas a pas:
 ...
 * gettext () simplement tradueix un msgid al seu corresponent msgstr "per a un idioma determinat. També hi ha la funció shorthand _ () que funciona de la mateixa manera;
 * ngettext () fa el mateix però amb regles plurals;
 * també hi ha dgettext () i dngettext (), que permeten substituir el domini per a una sola trucada. Més informació sobre la configuració del domini en el següent exemple.

### 2. Un fitxer de configuració d'exemple (i18n_setup.php tal com s'utilitza anteriorment), seleccionant la configuració local correcta i configurant Gettext ###
...
Un dels grans avantatges que Gettext té en els paquets i18n de framework personalitzat és el seu extens i potent format de fitxer. "Oh home, que és molt difícil d'entendre i editar a mà, una matriu senzilla seria més fàcil!" No us equivoqueu, aplicacions com Poedit són aquí per ajudar-vos-en molt. Podeu obtenir el programa des del lloc web, que és gratuït i disponible per a totes les plataformes. És una eina força fàcil per acostumar-se i alhora molt potent: utilitzar totes les funcions de què disposa Gettext. Aquesta guia es basa en PoEdit 2.2.

En el primer exercici, haureu de seleccionar "Fitxer> Nou ..." al menú. Se us demanarà de franc l’idioma: aquí podeu seleccionar / filtrar l’idioma que vulgueu traduir o utilitzar el format que hem esmentat abans, com ara en_US o pt_BR.

Ara, deseu el fitxer: també utilitzeu l'estructura de directoris que hem esmentat. Aleshores, haureu de fer clic a "Extreu de fonts" i aquí configurareu diversos paràmetres per a les tasques d'extracció i traducció. Podreu trobar-ne més endavant a través de "Catàleg> Propietats":

 * Rutes de la font: aquí heu d’incloure totes les carpetes del projecte on s’anomenen gettext () (i germans): normalment és la vostra carpeta de plantilles / visualitzacions. Aquest és l’únic paràmetre obligatori;
 * Propietats de la traducció:
    * Nom i versió del projecte, adreça de correu electrònic de Team and Team: informació útil que es troba a la capçalera del fitxer .po;
    * Formes plurals: aquí es mostren les regles que hem esmentat abans; també hi ha un enllaç amb mostres. Podeu deixar-ho amb l'opció predeterminada la majoria de les vegades, ja que PoEdit ja inclou una base de dades útil de regles plurals per a molts idiomes.
   * Xats: UTF-8, preferentment;
   * Característica del codi font: heu de configurar aquí el conjunt utilitzat per la base de codi, probablement també UTF-8, oi?
...
Després d’establir aquests punts, s’executarà una exploració a través dels fitxers font per trobar totes les trucades de localització. Després de cada exploració, PoEdit mostrarà un resum del que s'ha trobat i del que s'ha tret dels fitxers d'origen. Les entrades noves s’enviaran buides a la taula de traducció i començareu a escriure les versions localitzades d’aquestes cadenes. Desa-ho i es recopilarà un fitxer .mo a la mateixa carpeta i ta-dah: el teu projecte s’internacionalitza.
...
Si utilitzeu PHP com a mòdul a Apache (mod_php), podríeu tenir problemes amb el fitxer .mo en memòria cau. El primer cop es llegeix i, per actualitzar-lo, potser haureu de reiniciar el servidor. A Nginx i PHP5 solen prendre un parell de actualitzacions de pàgines per actualitzar la memòria cau de traducció, i en PHP7 poques vegades es necessita.

#### Funcions addicionals d’assistència ####

Tal com prefereix molta gent, és més fàcil utilitzar _ () en lloc de gettext (). Moltes biblioteques personalitzades de i18n també des de marcs usen quelcom semblant a t (), per fer més breu el codi traduït. Tanmateix, aquesta és l'única funció que ofereix una drecera. Potser voldreu afegir al projecte alguns altres, com ara __ () o _n () per a ngettext (), o potser una fantasia _r () que uniria les trucades a gettext () i a sprintf (). Altres biblioteques, com ara Gettext d’oscarotero, també ofereixen funcions d’ajuda com aquestes.

En aquests casos, haureu d’instruir l’utilitat de Gettext sobre com extreure les cadenes d’aquestes funcions noves. No tingueu por; és molt fàcil. Es tracta només d’un camp del fitxer .po o d’una pantalla de configuració de Poedit. A l'editor, aquesta opció es troba dins de "Catàleg> Propietats> Paraules clau de font". Recordeu-ho: Gettext ja coneix les funcions predeterminades de molts idiomes, així que no tingueu por de si aquesta llista sembla buida. Heu d’incloure aquí les especificacions d’aquestes funcions noves, seguint un format específic:
 ...
 * si la funció té més d’un argument, podeu especificar en quin és la primera cadena i, si cal, també la forma plural. Per exemple, si anomenem la nostra funció així: __ ('un usuari', '% d usuaris', $ number), l'especificació seria __: 1,2, és a dir, el primer formulari és el primer argument i el segon. la forma és el segon argument. Si el vostre número apareix com a primer argument, l’especificació seria __: 2,3, indicant que el primer formulari és el segon argument, etc.

Després d’incloure aquestes noves regles al fitxer .po, una nova exploració comportarà les vostres cadenes tan fàcilment com abans.


# Referències #

 * Viquipèdia: i18n i l10n
 * Viquipèdia: Gettext
 * LingoHub: internacionalització de PHP amb tutorial de gettext
 * Manual PHP: Gettext
 * Manual de Gettext
